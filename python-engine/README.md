...
Stdout/stderr share 100,000 UTF-16 code units and truncate at the Worker boundary. Python exception text, diagnostics and each inspection text field are capped at 100,000 characters. Tokens are limited to 1,500 entries and 1,000 characters per value; AST nodes to 500 with eight scalar detail fields each; code objects to 40 with 200 constants/names/locals each; instructions to 4,000. Nested code inspection is limited to depth 12. Token/instruction truncation is explicit; AST traversal text marks omitted nodes. Matching result messages are limited to 4,000,000 serialized characters; an oversized or malformed result fails the operation and replaces the runtime. Package requests accept at most the nine curated IDs before deduplication. These are output and protocol bounds, not a browser memory quota; large Python allocations and some intermediate formatting work can still exhaust a tab.
