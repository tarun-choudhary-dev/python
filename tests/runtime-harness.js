...
export async function runRuntimeTests() {
  ...
  async function inspect(source) { const result = wait('result', ++id); await runtime.run(id, source, 'inspect'); return result; }
  ...
  try {
    ...
    let inspectorFetches = 0;
    window.fetch = (input, ...options) => {
      if (String(input).endsWith('/inspector.py')) {
        inspectorFetches++;
        throw new Error('Inspection is deliberately unavailable during execution');
      }
      return originalFetch(input, ...options);
    };
    let isolated = await run('print("execution only")');
    assert(isolated.stdout === 'execution only\n' && !isolated.error && !Object.hasOwn(isolated, 'tokens') && inspectorFetches === 0,
      'run succeeds without loading or producing inspection artifacts');
    window.fetch = originalFetch;
...
print(Image.new("RGB", (1, 1), "red").getpixel((0, 0)))`);
    ...
    for (const [source, error] of [['def :', 'SyntaxError'], ['print(missing_name)', 'NameError'], ['1 + "a"', 'TypeError'], ['import no_such_pylab_module', 'ModuleNotFoundError'], ['1 / 0', 'ZeroDivisionError']]) {
      ...
      if (error === 'SyntaxError') assert(!Object.hasOwn(r, 'bytecode') && r.errorKind === 'syntax', 'run syntax errors have no inspection artifacts');
    }
    ...
    assert(r.stdout === '8\n' && !Object.hasOwn(r, 'bytecode'), 'nested Python code executes without disassembly');
    ...
    r = await inspect('def double(value):\n    return value * 2\nclass Thing:\n    def size(self):\n        return 3\nprint(double(4))');
    assert(!r.stdout && r.astTree.includes('FunctionDef (name=\'double\')') && r.astTree.includes('ClassDef (name=\'Thing\')'), 'nested function and class AST without execution');
    ...
    r = await inspect('total = 0\nfor i in range(3):\n    total += i\nprint(total)');
    assert(!r.stdout && r.astTree.includes('For') && r.bytecode.includes('FOR_ITER'), 'loops appear through AST and bytecode without execution');
    r = await inspect('import math\nprint(math.factorial(4))');
    assert(!r.stdout && r.astTree.includes('Import') && r.codeObject.includes('math') && r.bytecode.includes('IMPORT_NAME'), 'standard-library imports are inspected without execution');
    r = await inspect('café = "λ🙂"\nprint(café)');
    assert(!r.stdout && r.tokens.some(item => item.value === 'café') && r.tokens.some(item => item.type === 'STRING' && item.value === '"λ🙂"') && r.astDump.includes('café'), 'Unicode identifiers and strings survive inspection');

    r = await inspect('print(');
    ...
    r = await inspect('print("before")\nraise ValueError("after")');
    assert(!r.stdout && r.astTree.includes('Raise') && r.codeObject && r.bytecode && r.disassembly && !r.error, 'inspection does not execute a runtime exception');
    r = await inspect('print("clean")');
    assert(!r.stdout && !r.astError && !r.compileError && !r.tokenError && r.astTree, 'subsequent inspection clears prior syntax errors');
    r = await inspect('a = 1\n'.repeat(600));
    ...
    r = await inspect('for i in range(5):\n    if i % 2 == 0:\n        print(i)');
